                        ...
                        @can('admin')
                            ...
                            <li class="nav-item dropdown no-arrow mx-1">
                                <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button"
                                    ...
                                    <!-- Counter - Alerts -->
                                    <span class="badge badge-danger badge-counter">{{DB::table('order')->where('pack', '=', false)->count()}}+</span>
                                </a>
                                <!-- Dropdown - Alerts -->
                                <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
                                    aria-labelledby="alertsDropdown">
                                    <h6 class="dropdown-header">
                                        Alerts payment Center
                                    </h6>
                                    <?php
                                        $orders = DB::table('order')->where('pack', '=', false)->get();
                                    ?>
                                    ...
                                    @foreach ($orders as $order)
                                        ...
                                        <a class="dropdown-item d-flex align-items-center" href="#">
                                            ...
                                            <div>
                                                <div class="small text-gray-500">{{$order->reference}}</div>
                                                <span class="font-weight-bold">name: {{$order->name}}, email: {{$order->email}}</span>
                                            </div>
                                        </a>
                                    @endforeach
                                    <a class="dropdown-item text-center small text-gray-500" href="{{route('orders.index')}}">Show All order</a>
                                </div>
                            </li>
